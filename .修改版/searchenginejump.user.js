...
// @include        /^https?:\/\/zh\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/
// @include        /^https?:\/\/en\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/
// @include        /^https?:\/\/ja\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/
...
// @include        /^https?:\/\/mp3\.soso\.com\/music\.so/
...
// @include        /^https?:\/\/image\.soso\.com\/pics/
...
var rules = [
	...
	{name: "wiki[ZH]",
		url: /^https?:\/\/zh\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/,
		enabled: true,
		engineList: "知识",
		style: '\
		      border-top:1px solid #D9E1F7;\
	...
	{name: "wiki[EN]",
		url: /^https?:\/\/en\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/,
		enabled: true,
		engineList: "知识",
		style: '\
		      border-top:1px solid #D9E1F7;\
	...
	{name: "wiki[JP]",
		url: /^https?:\/\/ja\.wikipedia\.org\/(wiki\/|w\/index\.php\?search=%s)/,
		enabled: true,
		engineList: "知识",
		style: '\
		      border-top:1px solid #D9E1F7;\
	...
	{name: "搜搜音乐(搜狗马甲)",
	    url: /^https?:\/\/mp3\.soso\.com\/music\.so/,
	    enabled: true,
	    engineList: "music",
	    style: "\
	        text-align:left;\
	    ...
	    ",
	    insertIntoDoc: {
	        keyword: 'css;#queryinput_soso',
	        target: 'css;#header_soso',
	        where: 'afterEnd'
	    }
	},
	{name: "搜狗音乐",
	...
	{name: "音悦台",
	    ...
	    stylish: '.search { margin-top: 30px !important; }'
	},
	{name: "一听音乐",
...
var engineListDataStr = getMStr(function(){
	...
	/*
	网页
	    Google, http://www.google.com.hk/search?q=%s&ie=utf-8, data:image/png;base64,
	...
	购物
	    ...
	    QQ网购, http://se.wanggou.com/comm_search?KeyWord=%s, gbk, http://www.wanggou.com/favicon.ico
...
function parseDataStr(str) {
	...
	str.split(/[\n\r]+/).forEach(function(line){
		...
		var arr = line.replace(/，/g, ', ').split(/, /).map(function(s){
			return s && s.trim();
		});
		if (arr.length === 1) {
			type = line;
			...
			if (arr[2] && ['utf-8', 'gbk'].indexOf(arr[2].toLowerCase()) != -1) {
				engline.encoding = arr[2];
				engline.favicon = arr[3] || getFaviconFromUrl(arr[1]);
			} else {
				engline.favicon = arr[2] || getFaviconFromUrl(arr[1]);
			}
...
function getFaviconFromUrl(url) {
	...
	var link = getFaviconFromUrl.link;
	if (!link) {
...
function run() {
	...
	if (!iTarget || !iInput) {
		console.error('不存在插入的位置或匹配的输入框', iTarget, iInput);
		return;
	}

	// 添加全局样式和自定义样式
	var globalStyle = $('sej-style');
	if (!globalStyle) {
	...
	var engineList = parseDataStr(engineListDataStr);
	Object.keys(engineList).forEach(function (nameAndNum) {
		var cName = nameAndNum.split('-')[0],
			iconNum = nameAndNum.split('-')[1];
		var category = cName;

		var engines = [];

		engineList[nameAndNum].forEach(function (engine) {
			if (toRE(matchedRule.url).test(engine.url)) return;// 去掉跳转到当前引擎的引擎
		...
		if (isTheSameCategory(category, matchedRule.engineList)) {
			...
			// 更改图标为第几个
			if (iconNum) {
				var e = engineList[nameAndNum][iconNum - 1];
				if (e && e.favicon) {
...
if (matchedRule.mutationTitle) {
	// 监视标题的变化
	debug('添加节点监视器: title');
	addMutationObserver('title', function(){
...
var test = {
	...
	// 有问题，只能得到一个？
	iconToBase64: function() {
		var list = parseDataStr(engineListDataStr);

		Object.keys(list).forEach(function(category){
			list[category].forEach(function(engline){
				if (engline.favicon.indexOf('http') === 0) {
					console.debug(engline.favicon);
				}
			});
		});
	},
	// 转换 engineList 为 engineListDataStr
	convertEnglineListToStr: function () {
		var str = '';
		Object.keys(engineList).forEach(function(type){
		    str += type + '\n';

		    var list = engineList[type];
		    list.forEach(function(engine){
			   str += '    ' + engine.name + '  ' + engine.url + '  ' + (engine.encoding || '');
			   str += (getFaviconFromUrl(engine.url) == engine.favicon) ? '' : '  ' + engine.favicon;
			   str += '\n';
		    });
		});
		copy(str)
