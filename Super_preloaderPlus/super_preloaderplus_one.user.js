...
var SITEINFO=[
	{name: 'Google搜索',                                                                     // 站点名字...(可选)
		...
		autopager:{
			...
			lazyImgSrc: 'imgsrc',                                  // 新增的自定义样式。下面这个是调整 Google 下一页可能出现的图片排列问题。
			mutationTitle: true,                                   // 监视标题节点的变化(特殊)
	...
	{name: 'Bing网页搜索',
		...
		autopager:{
			pageElement: '//ol[@id="b_results"]',
			documentFilter: function(doc){  // 修正下一页缺失的图片
				var snap = doc.evaluate('//script/text()[contains(self::text(), "data:image/")]', doc, null, 7, null);

				var _ge = function(id) {
					return doc.getElementById(id);
				};

				for (var i = snap.snapshotLength - 1, fnStr; i >= 0; i--) {
					fnStr = snap.snapshotItem(i).textContent;
					eval(fnStr);
				}
			}
		}
	...
	{name: '网易数码',
		url: /^http:\/\/hea\.163\.com\/.*\.html/i,
		exampleUrl: 'http://hea.163.com/14/1113/05/AATJSDSM001628C1.html#index_hea_3',
		nextLink: 'id("epContentLeft")/div[@class="ep-pages"]/a[@class="ep-pages-ctrl"]',
		autopager: {
			pageElement: 'id("endText")',
			relatedObj: true,
			replaceE: '//div[@class="ep-pages"]'
		}
	},
	{name: '新华网新闻页面',
	...
	// {name: '京东商城',  // 没有价格，且官方搜索（ajax 方式）不支持
	// 	url: /^http:\/\/.*\.jd\.com\//i,
	...
	{name: '知乎',
		...
		autopager: {
			...
			replaceE: '//div[@class="zm-invite-pager"]',
		}
	},
	{name: '译言网 | 译文库和原文库',
	...
	{name: '天涯论坛帖子',
		...
		autopager:{
			...
			documentFilter: function(doc) {
				// 只看楼主的隐藏
				var link = document.querySelector('.see-host.see-host-off');
				if (link) {
					var uname = link.getAttribute('_uname');
					[].forEach.call(doc.querySelectorAll('.atl-item'), function(node) {
						if (node.getAttribute('js_username') != uname) {
	...
	{name: '卡饭',
		url: /^http:\/\/bbs\.kafan\.cn\//i,
		exampleUrl: 'http://bbs.kafan.cn/forum-215-1.html',
		...
		autopager: {
			...
			stylish: '.spl-bbscode { font-family:Monaco,Consolas,"Lucida Console","Courier New",serif;font-size:12px;line-height:1.8em;list-style-type:decimal-leading-zero;padding-left:10px;background:none repeat scroll 0 0 #f7f7f7;color:#666;border:1px solid #ccc;overflow:hidden;padding:10px 0 5px 10px}',
			documentFilter: function(doc) {
				// 卡饭论坛的下一页代码区域可能无法着色，所以手动修改并添加样式
				var pres = doc.querySelectorAll('pre[class^="brush:"]');
				[].forEach.call(pres, function(pre){
					pre.classList.add('spl-bbscode');
				});
			}
		}
	},
	...
	{name: '优美高清美女图片',
		url: /^http:\/\/www\.umei\.cc\/.*\.htm/,
		exampleUrl: 'http://www.umei.cc/p/gaoqing/gangtai/20141031193302.htm',
		nextLink: '//div[@class="pages"]/a[text()="下一页"]',
		autopager: {
			pageElement: ['//div[@id="IMG_Zoom"]', '//div[@id="content"]'],
			documentFilter: function(doc) {
				var oImg = document.querySelector('.IMG_show');
				if (!oImg) return;

				var oWith = oImg.width;
				var images = doc.querySelectorAll('.IMG_show');
	...
	{name: 'reddit.com: the front page of the internet',
		url: /^https?:\/\/(?:[^.]+\.)?reddit\.com\//i,
		exampleUrl: 'http://www.reddit.com/',
		nextLink: '//span[@class="nextprev"]/a[contains(concat(" ",normalize-space(@rel)," "), " next ")]',
		autopager: {
			pageElement: '(.//*[@id="siteTable"])[last()]',
		}
	},
...
var SITEINFO_TP=[
	...
	{name: 'Discuz论坛列表',
		...
		autopager:{
			pageElement:'//div[@id="postlist"] | //form[@method="post"][@name]',
			replaceE: '//div[@class="pages" or @class="pg"][child::a[@class="next" or @class="nxt"][@href]]',
			lazyImgSrc: 'file|zoomfile|pagespeed_lsc_url'
		}
	},
	{name: 'Discuz论坛帖子',
