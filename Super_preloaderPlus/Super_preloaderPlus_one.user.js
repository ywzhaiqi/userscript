...
var prefs={
    ...
    // 新增的
    autoGetPreLink: false,   // 一开始不检测上一页链接，调用时再检测
...
var SITEINFO=[
    ...
    // ========================= 有问题 ================================================
    {name: "taobao 搜索",
        enable: false,  // Chrome 下 Super_preloader 在淘宝有问题
    ...
    // 软件论坛
    {name: "卡饭论坛 - 导读-最新热门",
        url: "^http://bbs\\.kafan\\.cn/forum\\.php\\?mod=guide",
...
(function() {
    ...
    // Firefox 下比小说脚本早运行，而 Chrome TM 下会迟点
    if(browser.firefox){
        setTimeout(run, 100);
    }else{
        run();
    ...
    function init(browser, window, document, db) {
        ...
        function autopager(SSS, floatWO) {
            ...
            function createSep(lastUrl, currentUrl, nextUrl) {
                ...
                if (SSS.a_separator) {
                    ...
                    var pageStr = '第 <span style="color:red!important;">' + curNumber + '</span> 页' +
                            getRalativePageStr(lastUrl, currentUrl, nextUrl);
                    div.appendChild($C('a', {
            ...
            function afterInsertPage(docTitle) {
...
function browserCompatible() {
    if(Array.slice === undefined){  // FF 下有，Chrome 下没有，TM 做了兼容
