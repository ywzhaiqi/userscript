...
var moveDialog = {
    ...
    initUI: function() {
        $('<style>')
            .text([
                ...
                '#bph-custom-path-input { width: 300px; }',
                '.autocomplete-suggestions { background: #fff; padding-left: 5px;  }',
            ].join('\n'))
            .appendTo('head');

        // 添加路径编辑器
        moveDialog.$input = moveDialog.createInput();

        $('.move-dialog h3')
            .append(moveDialog.createCheckbox())
            .append(moveDialog.$input);
    },
    createCheckbox: function() {
        ...
        var checkboxClicked = function() {
            if (this.checked) {
                var path = moveDialog.$input.val();
                if (path) {
                    fileTreeView.obtain.getSelectPath = function() {
                        return path;
                    };
                } else {
                    moveDialog.$input.focus();
                }
            } else {
    ...
    createInput: function() {
        ...
        var paths = [
            { value: currentDir, data: currentDir },
        ];

        var dirNames = moveDialog._getCurDirItems();
        $(dirNames).each(function(i, dirName) {
            paths.push({
                value: currentDir + '/' + dirName,
                data: dirName
            })
        });
